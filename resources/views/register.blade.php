...
<script type="text/javascript">
  ...
  $('.bk_validate_code').click(function () {
      $(this).attr('src', 'service/validate_code/create?' + Math.random());
  });
  ...
  $('.bk_phone_code_send').click(function(event) {
    ...
    var interval = window.setInterval(function() {
      ...
      if(num == 0) {
        ...
        enable = true; // 可以点击发送按钮
        window.clearInterval(interval); //销毁定时器
    ...
    $.ajax({
      ...
      success: function(data) {
        if(data == null) { // 没有数据返回，表示服务器端错误
          $('.bk_toptips').show();
          $('.bk_toptips span').html('服务端错误');
          setTimeout(function() {$('.bk_toptips').hide();}, 4000);
          return;
        }
        if(data.status != 0) { //有错误，返回错误信息
          $('.bk_toptips').show();
